from common.neptune_utils import node_result
from question_answering.strategies.base_strategy import BaseStrategy
from question_answering.strategies.facts import FactSimilaritySearch, FactExpansion
...
class GetChunks(BaseStrategy):
    def __init__(self, neptune_client, results_accessors=[ChunkSimilaritySearch], top_n=5):
        self.neptune_client = neptune_client
        self.top_n = top_n
    ...
    def accept(self, context):
        ...
        for results_accessor in self.results_accessors:
            chunk_ids.extend(results_accessor.get_results(context, selector='chunk/chunkId'))
        ...
        params = {
            'chunk_ids': chunk_ids[:self.top_n]
        }
...
class RerankChunks(BaseStrategy):
    def __init__(self, results_accessors=[ChunkSimilaritySearch, FactSimilaritySearch, FactExpansion]):
        self.results_accessors = results_accessors
    ...
    def accept(self, context):
        ...
        for rescored_result in rescored_results:
            ...
            if chunk['chunkId'] not in reranked_results:
                ...
                new_score = reranked_results[chunk['chunkId']]['score'] + rescored_result['score']
