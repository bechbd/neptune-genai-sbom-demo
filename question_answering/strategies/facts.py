...
class FactExpansion(BaseStrategy):
    ...
    def accept(self, context):
        ...
        def is_new_fact(new_result):
            return new_result['fact']['factId'] not in [result['fact']['factId'] for result in results]
        
        fact_ids = []
        for results_accessor in self.results_accessors:
            fact_ids.extend(results_accessor.get_results(context, selector='fact/factId'))
        
        
        for i in range (0, self.max_depth):
            
            new_facts = list(filter(is_new_fact, self.top_k_facts([embedding], fact_ids)))
